...
// =====================================================================
{
  ...
  const rows = [
    ["Normal — عادی", "148", "150"],
    ["Robbery — سرقت", "23", "≈ 5"],
    ["RoadAccidents — تصادف", "23", "≈ 23"],
    ["Stealing — دزدی", "15", "≈ 5"],
    ["Burglary — سرقت از منزل", "15", "≈ 5"],
    ["Shoplifting — دزدی از مغازه", "8", "≈ 5"],
    ["Fighting — درگیری", "8", "≈ 5"],
    ["Assault — تعرض", "8", "≈ 5"],
    ["Vandalism — تخریب", "8", "≈ 5"],
    ["Abuse — سوءاستفاده", "8", "2"],
    ["سایر کلاس‌ها", "8 هرکدام", "≈ 5"],
  ];
  table(s, 0.6, 1.75, 7.4, ["کلاس", "تقسیم مسیر اول", "تقسیم استاندارد"],
        rows, { colW: [3.6, 1.9, 1.9], rowH: 0.31, size: 11.5 });

  card(s, 8.3, 1.75, 4.4, 2.4, "FFF7EA");
  s.addText("دو تقسیم متفاوت", rtl({
    x: 8.6, y: 1.95, w: 3.8, h: 0.35, fontSize: 16, bold: true,
    color: AMBER, margin: 0,
  }));
  s.addText(
    "مسیر X-CLIP روی تقسیمی با ۲۹۶ ویدیوی آزمون اجرا شد و مسیر VideoMAE روی تقسیم استاندارد با ۲۹۰ ویدیو. به همین دلیل اعداد دو مسیر مستقیماً قابل جمع نیستند.",
    rtl({ x: 8.6, y: 2.4, w: 3.8, h: 1.6, fontSize: 12, color: INK,
          lineSpacing: 19, margin: 0 }));

  card(s, 8.3, 4.35, 4.4, 2.3);
  s.addText("چرا ارزیابی نوفه‌آلود است", rtl({
    x: 8.6, y: 4.55, w: 3.8, h: 0.35, fontSize: 16, bold: true,
    color: NAVY, margin: 0,
  }));
  s.addText(
    "با هشت ویدیو در یک کلاس، تغییر برچسب یک ویدیو ۱۲٫۵ درصد یادآوری آن کلاس را جابه‌جا می‌کند. ماکرو F1 به هر ۱۴ کلاس وزن یکسان می‌دهد.",
    rtl({ x: 8.6, y: 5.0, w: 3.8, h: 1.5, fontSize: 12, color: INK,
          lineSpacing: 19, margin: 0 }));

  stat(s, 0.6, 5.5, 2.4, "1610", "ویدیوی آموزش", NAVY);
  stat(s, 3.1, 5.5, 2.4, "290", "ویدیوی آزمون", NAVY);
  stat(s, 5.6, 5.5, 2.4, "0%", "همپوشانی دو مجموعه", TEAL);
  s.addText(
    "مجموعه آزمون تا آخرین مرحله دیده نمی‌شود: نه وزن‌ها روی آن آموزش می‌بینند و نه آستانه یا ابرپارامتری روی آن انتخاب می‌شود.",
    rtl({ x: 0.6, y: 6.65, w: 7.4, h: 0.5, fontSize: 11.5, color: MUTED,
