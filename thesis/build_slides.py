...
# Vazir must be installed on the presenting machine; PowerPoint
# substitutes silently when it is not.
FA = "Vazir"
...
textbox(s, 0.8, 4.15, 8.4, 0.5,
        "یادگیری چندنمونه‌ای ضعیف‌نظارت با ستون فقرات مبدل ویدیویی",
        size=16, color=ICE)
...
# 4 — X-CLIP splits
# =====================================================================
s = light_slide("پروتکل تقسیم داده — مسیر اول", "X-CLIP")
textbox(s, 0.6, 1.7, 12.1, 0.36,
        "تفکیک ۷۰ به ۱۵ به ۱۵ طبقه‌ای روی کل مجموعه، سپس متوازن‌سازی "
        "مجموعه آموزش با بیش‌نمونه‌برداری کلاس‌های اقلیت.", size=12.5,
        color=MUTED)

XCLIP_TRAIN = [
    ["Normal", "420"], ["Burglary", "140"], ["RoadAccidents", "140"],
    ["Robbery", "140"], ["Shooting", "140"], ["Stealing", "140"],
    ["Vandalism", "140"], ["Abuse", "105"], ["Arrest", "105"],
    ["Arson", "105"], ["Assault", "105"], ["Explosion", "105"],
    ["Fighting", "105"], ["Shoplifting", "105"],
]
XCLIP_TEST = [
    ["Normal", "148"], ["RoadAccidents", "23"], ["Robbery", "23"],
    ["Burglary", "15"], ["Stealing", "15"], ["Abuse", "8"],
    ["Arrest", "8"], ["Arson", "8"], ["Assault", "8"],
    ["Explosion", "8"], ["Fighting", "8"], ["Shooting", "8"],
    ["Shoplifting", "8"], ["Vandalism", "8"],
]

table(s, 0.6, 2.15, 5.9, ["کلاس", "مجموعه آموزش"], XCLIP_TRAIN,
      [3.7, 2.2], row_h=0.26, size=11, head_size=11.5)
table(s, 6.8, 2.15, 5.9, ["کلاس", "مجموعه آزمون"], XCLIP_TEST,
      [3.7, 2.2], row_h=0.26, size=11, head_size=11.5)

stat(s, 0.6, 6.15, 2.9, "1995", "اندازه‌ی مؤثر مجموعه آموزش")
stat(s, 3.6, 6.15, 2.9, "296", "اندازه‌ی مجموعه آزمون")
textbox(s, 6.8, 6.2, 5.9, 0.85,
        "اعداد ستون آموزش تعداد نمونه‌های ارائه‌شده به بهینه‌ساز است، نه "
        "تعداد ویدیوی یکتا؛ کلاس‌های کم‌بسامد با جایگذاری بیش‌نمونه‌برداری "
        "شده‌اند.", size=11, color=MUTED, spacing=16)
notes(s, "اعداد ستون آموزش مستقیم از create_balanced_split.py می‌آیند و "
         "قطعی‌اند. ستون آزمون از گزارش آزمایش‌ها بازسازی شده؛ پیش از "
         "ارائه با فایل تقسیم خودتان بسنجید.")

# =====================================================================
# 5 — VideoMAE splits
# =====================================================================
s = light_slide("پروتکل تقسیم داده — مسیر دوم", "VideoMAE")
textbox(s, 0.6, 1.7, 12.1, 0.36,
        "تفکیک استاندارد مرجع مجموعه‌ی UCF-Crime، بدون بازنمونه‌گیری و "
        "بدون متوازن‌سازی.", size=12.5, color=MUTED)

MAE_TRAIN = [
    ["Normal", "800"], ["Robbery", "145"], ["RoadAccidents", "127"],
    ["Stealing", "95"], ["Burglary", "87"], ["Abuse", "48"],
    ["Assault", "47"], ["Arrest", "45"], ["Fighting", "45"],
    ["Vandalism", "45"], ["Arson", "41"], ["Explosion", "29"],
    ["Shoplifting", "29"], ["Shooting", "27"],
]
MAE_TEST = [
    ["Normal", "150"], ["RoadAccidents", "23"], ["Shooting", "23"],
    ["Explosion", "21"], ["Shoplifting", "21"], ["Burglary", "13"],
    ["Arson", "9"], ["Arrest", "5"], ["Fighting", "5"],
    ["Robbery", "5"], ["Stealing", "5"], ["Vandalism", "4"],
    ["Abuse", "3"], ["Assault", "3"],
]

table(s, 0.6, 2.15, 5.9, ["کلاس", "مجموعه آموزش"], MAE_TRAIN,
      [3.7, 2.2], row_h=0.26, size=11, head_size=11.5)
table(s, 6.8, 2.15, 5.9, ["کلاس", "مجموعه آزمون"], MAE_TEST,
      [3.7, 2.2], row_h=0.26, size=11, head_size=11.5,
      highlight=(7, 8, 9, 10, 11, 12, 13))

stat(s, 0.6, 6.15, 2.9, "1610", "اندازه‌ی مجموعه آموزش")
stat(s, 3.6, 6.15, 2.9, "290", "اندازه‌ی مجموعه آزمون")
textbox(s, 6.8, 6.2, 5.9, 0.85,
        "ردیف‌های مشخص‌شده هفت کلاسی هستند که پشتیبان آماری آن‌ها در "
        "مجموعه‌ی آزمون پنج نمونه یا کمتر است؛ منشأ اصلی واریانس معیار "
        "ماکرو.", size=11, color=MUTED, spacing=16)
notes(s, "نسبت عدم توازن در این مسیر دست‌نخورده می‌ماند؛ به همین دلیل "
         "وزن‌دهی معکوس فراوانی در تابع زیان ضروری است.")

# =====================================================================
# 6 — the MIL formulation
# =====================================================================
s = light_slide("صورت‌بندی یادگیری چندنمونه‌ای", "چارچوب")
textbox(s, 0.6, 1.7, 12.1, 0.36,
        "نظارت در سطح کیف: برچسب تنها برای کل ویدیو موجود است و هیچ "
        "حاشیه‌نویسی زمانی در مجموعه‌ی آموزش وجود ندارد.", size=12.5,
        color=MUTED)

card(s, 0.6, 2.2, 6.1, 3.05, CREAM)
textbox(s, 0.95, 2.4, 5.4, 0.35, "تعریف صوری", size=15, bold=True,
        color=ACCENT)
textbox(s, 0.95, 2.85, 5.4, 2.2,
        ["هر ویدیو یک کیف است:",
         "X = { x₁ , … , x_T } ,   x_t ∈ ℝᴰ ,   T = 16 , D = 1024",
         "",
         "برچسب کیف Y ∈ { 0 , … , 13 } و فرض استاندارد چندنمونه‌ای:",
         "Y ≠ 0  ⟺  ∃ t :  نمونه‌ی x_t حامل رویداد کلاس Y است"],
        size=12, spacing=19, space_after=4)

card(s, 7.0, 2.2, 5.7, 3.05)
textbox(s, 7.35, 2.4, 5.0, 0.35, "تجمیع و تابع زیان", size=15, bold=True,
        color=TITLE)
textbox(s, 7.35, 2.85, 5.0, 2.2,
        ["شواهد ناهنجاری هر نمونه:",
         "s_t = 1 − P( Normal | x_t )",
         "",
         "انتخاب k نمونه‌ی برتر و تجمیع میانگین:",
         "K = argtop-k { s_t } ,   z = (1/k) Σ_{t∈K} f( x_t )",
         "",
         "زیان با وزن کلاس و دو جمله‌ی تنظیم:",
         "L = CE( z , Y ; w ) + λ_s Σ (a_{t+1} − a_t)² + λ_p Σ a_t"],
        size=11.5, spacing=18, space_after=3)

card(s, 0.6, 5.45, 12.1, 1.5, MINT)
textbox(s, 0.95, 5.62, 11.4, 0.35, "سهم روش‌شناختی این پژوهش", size=14,
        bold=True, color=TEAL)
textbox(s, 0.95, 6.0, 11.4, 0.85,
        "در فرمول‌بندی دودویی، انتخاب نمونه‌های برتر روی یک کانال انجام "
        "می‌شود. در تعمیم چهارده‌کلاسه، اعمال مستقل آن بر هر کانال سبب "
        "می‌شود کانال کلاس عادی از عادی‌ترین نمونه‌های یک کیف ناهنجار "
        "ساخته شود و نوفه‌ی برچسب سامانمند وارد آموزش گردد. ما رتبه‌بندی "
        "را یک بار بر پایه‌ی s_t انجام می‌دهیم و همان زیرمجموعه را برای "
        "تمام کانال‌ها به کار می‌بریم.", size=11.5, spacing=17)
notes(s, "این اسلاید صورت‌بندی رسمی است. اگر وقت کم آمد، فقط فرض "
         "چندنمونه‌ای و اصلاح انتخاب یک‌باره را بگویید.")
...
# 18 — temporal head
# =====================================================================
s = light_slide("سر مبدل روی محور زمان", "مسیر دوم")
...
table(s, 0.6, 1.8, 12.1,
      ["پیکربندی", "رمزگذار", "ماکرو F1", "صحت نوع", "صحت کلی", "AUC"], [
          ...
          ["مسیر دوم — سر نقطه‌ای، خط پایه", "VideoMAE ViT-B", "0.2975",
           "0.3860", "0.5660", "—"],
          ["مسیر دوم — سر مبدل زمانی", "VideoMAE ViT-B", "0.3444",
           "0.3930", "0.6350", "—"],
...
for index, (label, body, colour) in enumerate([
        ("دستاورد",
         ...
         "مبدل زمانی، و پروتکل ارزیابی با کف نوفه", GOOD_TINT),
        ("محدودیت",
