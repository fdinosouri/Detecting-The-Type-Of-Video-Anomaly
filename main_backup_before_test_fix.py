    ...
    if config.TEST.ONLY_TEST:
        if not os.path.isdir(config.MODEL.PRETRAINED):
            ...
            try:
                ...
                logger.info(f"AUC@all/ano on test_scores.pkl : {auc_all:.4f}({auc_ano:.4f})")
...
def train_one_epoch(epoch, model, optimizer, lr_scheduler, train_loader, text_labels, config, data_dict=None):
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        logits_video = torch.gather(
            logits,
            1,
            max_ind[:, None, None].repeat((1, 1, config.DATA.NUM_CLASSES))
        ).squeeze(1)
        ...
        if idx == 0 and epoch == 0:
            ...
            print("DEBUG label_id min/max:", label_id.min().item(), label_id.max().item())
            print("DEBUG NUM_CLASSES:", config.DATA.NUM_CLASSES)                          
        # MIL loss
        loss_mil = F.cross_entropy(logits_video, label_id.long(), reduction='none')
