...
def index_frame_dirs(root, pattern):
    """Map video name -> (relative path, frame count) for every frame dir."""
...
def convert(src, dst, index, is_test):
    written = 0
    missing = []
    per_class = Counter()
    ...
    return written, missing, per_class
...
def main():
    ...
    print(f"Scanning {args.root} for frame directories ...", flush=True)
    index = index_frame_dirs(args.root, args.pattern)
    print(f"  found {len(index)} frame directories")
    ...
    for src, name, is_test in (
        ...
        dst = args.out / name
        written, missing, per_class = convert(src, dst, index, is_test)
        total_missing += missing

        print()
        print(f"{name}: wrote {written} entries, skipped {len(missing)}")
    ...
    if total_missing:
        ...
        print(f"\n{len(total_missing)} videos had no frame directory; "
              f"list written to {report}")
        print("Extract their frames, or accept the reduced split.")
